...
const Category = () => {
  ...
  const experienceList = async (page = 1) => {
    try {
      ...
      setState({
        loading: false,
        experienceList: tableData,
        count: res?.count,
      });
    } catch (error) {
      setState({ loading: false });
      Failure("Failed to fetch experiences");
    }
  };

  console.log("experienceList", state?.experienceList);

  const handlePageChange = (pageNumber: number) => {
    setState({ page: pageNumber });
    experienceList(pageNumber);
  };
      ...
      <div className="mb-4">
        <div className="flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between">
          <div className="space-y-2">
            <h1 className="page-ti text-transparent"> Experience Management</h1>
