...
const JobType = () => {
  ...
  useEffect(() => {
    dispatch(setPageTitle("Job Type Management"));
    jobTypeList(1);
  }, [dispatch]);
  ...
  const jobTypeList = async (page) => {
    try {
      ...
      const res: any = await Models.master.job_type_list(body);
      ...
      setState({
        loading: false,
        jobTypeList: tableData,
        count: res?.count,
      });
    } catch (error) {
      setState({ loading: false });
      Failure("Failed to fetch job types");
    }
  };

  const handlePageChange = (pageNumber: number) => {
    setState({ page: pageNumber });
    jobTypeList(pageNumber);
  };
  ...
  const deleteRecord = async (id: number) => {
    try {
      await Models.master.delete_job_type(id);
      Success("Job type deleted successfully!");
      jobTypeList(state.page);
    } catch (error) {
      Failure("Failed to delete job type");
    }
  };
  ...
  const handleSubmit = async () => {
    try {
      ...
      if (!state.name) {
        setState({ errors: { name: "Job type name is required" } });
        return;
      }

      if (state.editId) {
        await Models.master.update_job_type(body, state.editId);
        Success("Job type updated successfully!");
      } else {
        await Models.master.create_job_type(body);
    ...
    <div className="min-h-screen bg-gradient-to-br from-slate-50 to-blue-50 p-3 dark:from-gray-900 dark:to-gray-800">
      <div className="mb-8">
        <div className="flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between">
          ...
          >
            ...
            <span className="relative z-10">Add Job Type</span>
          </button>
        </div>
      </div>
      ...
      <div className="overflow-hidden rounded-2xl border border-gray-200 bg-white shadow-lg backdrop-blur-sm dark:border-gray-700 dark:bg-gray-800">
        ...
        <div className="overflow-x-auto">
          <DataTable
            noRecordsText="No job types found"
            highlightOnHover
            className="table-hover whitespace-nowrap"
            records={state.jobTypeList}
          ...
          <div className="relative">
            <div className="mb-8 text-center">
              ...
              <h2 className="text-2xl font-bold text-gray-900 dark:text-white">
                {state.editId ? "Update" : "Add New"} Job Type
              </h2>
            </div>
...
export default PrivateRouter(JobType);
