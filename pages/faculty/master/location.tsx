...
const Location = () => {
  ...
  const locationList = async (page) => {
    try {
      ...
      Failure("Failed to fetch locations");
    }
  };

  const handlePageChange = (pageNumber: number) => {
    setState({ page: pageNumber });
    locationList(pageNumber);
  };
  ...
  const deleteRecord = async (id: number) => {
    try {
      await Models.master.delete_location(id);
      Success("Location deleted successfully!");
      locationList(state.page);
    } catch (error) {
  ...
  const handleSubmit = async () => {
    try {
      ...
      const body = { city: state.city };

      if (!state.city) {
        setState({ errors: { city: "City name is required" } });
        return;
      }

      if (state.editId) {
        await Models.master.update_location(body, state.editId);
        Success("Location updated successfully!");
      } else {
        await Models.master.create_location(body);
  ...
  return (
    <div className="min-h-screen dark:from-gray-900 dark:to-gray-800">
      ...
      <div className="mb-4">
        <div className="flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between">
          <div className="space-y-2">
            <h1 className="page-ti text-transparent"> Location Management</h1>
      ...
      <Modal
        ...
        subTitle={`${state.editId ? "Update" : "Add New"} Location`}
        closeIcon
        renderComponent={() => (
          <div className="relative">
            {/* <div className="mb-8 text-center">
              ...
              <h2 className="text-2xl font-bold text-gray-900 dark:text-white">
                {state.editId ? "Update" : "Add New"} Location
              </h2>
            </div> */}
            ...
            <div className="space-y-6">
              <TextInput
                ...
                value={state.city}
                onChange={(e) => setState({ city: e.target.value, errors: { ...state.errors, city: "" } })}
...
export default PrivateRouter(Location);
