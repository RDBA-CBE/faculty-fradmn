...
const NIRFBand = () => {
  ...
  useEffect(() => {
    dispatch(setPageTitle("NIRF Band Management"));
    statusList(1);
  }, [dispatch]);
  ...
  const handleDelete = (row) => {
    showDeleteAlert(
      ...
      "Are you sure you want to delete this NIRF Band?"
    );
  };

  const deleteRecord = async (id: number) => {
    try {
      await Models.master.delete_NIRF_Band(id);
      Success("NIRF Band deleted successfully!");
  ...
  const handleSubmit = async () => {
    try {
      ...
      const body = { band: capitalizeFLetter(state.name) };

      if (!state.name) {
        setState({ errors: { name: "NIRF Band is required" } });
        return;
      }

      if (state.editId) {
        await Models.master.update_NIRF_Band(body, state.editId);
        Success("NIRF Band updated successfully!");
      } else {
        await Models.master.create_NIRF_Band(body);
        Success("NIRF Band created successfully!");
      }
  ...
  return (
    <div className="min-h-screen dark:from-gray-900 dark:to-gray-800">
      ...
      <div className="mb-4">
        <div className="flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between">
          <div className="space-y-2">
            <h1 className="page-ti text-transparent"> NIRF Bands</h1>
          ...
          >
            ...
            <span className="relative z-10">Add NIRF Band</span>
          </button>
        </div>
      </div>
      ...
      <Modal
        ...
        subTitle={`${state.editId ? "Update" : "Add New"}  NIRF Band`}
        closeIcon
        renderComponent={() => (
          <div className="relative">
            {/* <div className="mb-8 text-center">
              ...
              <h2 className="text-2xl font-bold text-gray-900 dark:text-white">
                {state.editId ? "Update" : "Add New"} NIRF Band
              </h2>
            </div> */}
...
export default PrivateRouter(NIRFBand);
