...
const SalaryRange = () => {
  ...
  useEffect(() => {
    dispatch(setPageTitle("Salary Range Management"));
    salaryRangeList(1);
  }, [dispatch]);
  ...
  const salaryRangeList = async (page) => {
    try {
      ...
      const res: any = await Models.master.salary_range_list(body);
      ...
      setState({
        loading: false,
        salaryRangeList: tableData,
        count: res?.count,
      });
    } catch (error) {
      setState({ loading: false });
      Failure("Failed to fetch salary ranges");
    }
  };

  const handlePageChange = (pageNumber: number) => {
    setState({ page: pageNumber });
    salaryRangeList(pageNumber);
  };
  ...
  const handleDelete = (row) => {
    showDeleteAlert(
      ...
      "Are you sure you want to delete this salary range?"
    );
  };

  const deleteRecord = async (id: number) => {
    try {
      await Models.master.delete_salary_range(id);
      Success("Salary range deleted successfully!");
      salaryRangeList(state.page);
    } catch (error) {
      Failure("Failed to delete salary range");
    }
  };
  ...
  const handleSubmit = async () => {
    try {
      ...
      if (!state.name) {
        setState({ errors: { name: "Salary range is required" } });
        return;
      }

      if (state.editId) {
        await Models.master.update_salary_range(body, state.editId);
        Success("Salary range updated successfully!");
      } else {
        await Models.master.create_salary_range(body);
        Success("Salary range created successfully!");
      }
  ...
  return (
    <div className="min-h-screen dark:from-gray-900 dark:to-gray-800">
      ...
      <div className="mb-4">
        <div className="flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between">
          ...
          >
            ...
            <span className="relative z-10">Add Salary Range</span>
          </button>
        </div>
      </div>
      ...
      <Modal
        ...
        subTitle={`${state.editId ? "Update" : "Add New"} Salary Range`}
        closeIcon
        renderComponent={() => (
          <div className="relative">
            {/* <div className="mb-8 text-center">
              ...
              <h2 className="text-2xl font-bold text-gray-900 dark:text-white">
                {state.editId ? "Update" : "Add New"} Salary Range
              </h2>
            </div> */}
...
export default PrivateRouter(SalaryRange);
