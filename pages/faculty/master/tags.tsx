...
const Tags = () => {
  ...
  useEffect(() => {
    dispatch(setPageTitle("Tags Management"));
    tagsList(1);
  }, [dispatch]);
  ...
  const tagsList = async (page) => {
    try {
      ...
      const res: any = await Models.master.tags_list(body);
  ...
  const deleteRecord = async (id: number) => {
    try {
      await Models.master.delete_tag(id);
  ...
  const handleSubmit = async () => {
    try {
      ...
      if (!state.name) {
        setState({ errors: { name: "Tag name is required" } });
        return;
      }

      if (state.editId) {
        await Models.master.update_tag(body, state.editId);
        Success("Tag updated successfully!");
      } else {
        await Models.master.create_tag(body);
      ...
      <div className="mb-8">
        <div className="flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between">
          ...
          >
            ...
            <span className="relative z-10">Add Tag</span>
          </button>
        </div>
      </div>
          ...
          <div className="relative">
            <div className="mb-8 text-center">
              ...
              <h2 className="text-2xl font-bold text-gray-900 dark:text-white">
                {state.editId ? "Update" : "Add New"} Tag
              </h2>
            </div>
