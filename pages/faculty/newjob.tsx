...
export default function Newjob () {
  ...
  const profile = async () => {
    try {
      ...
      setState({ profile: res, profile_institution: 9, institution: 9,profile_college:1 })
      if (res?.institution_id) {
        fetchColleges(9, 1)
      }
      if (res?.college_id) {
        setState({college:{value:1,label:1}})
        fetchDepartments(res?.college_id, 1)
      }
    } catch (error) {
